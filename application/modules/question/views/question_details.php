...
$(document).ready(function() {
	...
	$(document).on('click','.single-ans-submit',function() {
 		...
 		$.ajax({
 			...
 			success: function(response) { 
 				if(response.STATUS == "200") { 
 					...
 					if( response.AJAX_RES.subject == "fill_fields" ) {
 						var errorMessage 	=	response.AJAX_RES.data;
 						$("#user_answer_console").html(errorMessage);
 	...
 	$(document).on('click','.single-ans-submit-latex',function() {
 		loginRequiredPopup(is_user_logged);
 		var link_url		=	base_url+"question";
 		var questionId 		=	$(this).attr('btn-question-id');
 		var answerText 		=	$("#answer_text_latex").val();
 		var post_data		=	[
	 		{name: 'AJAX',value: 'AJAX'},
	 		{name: 'TYPE',value: 'ACTION'},
	 		{name: 'ACTION',value: 'ADD_ANSWER'},
	 		{name: 'QUESTION_ID',value: questionId},
	 		{name: 'answer_text',value:answerText},
	 		{name: 'mimeType',value:"application/x-latex"}
 		];
 	...
 	$(document).on('click','.single-ans-submit-plain',function() {
 		loginRequiredPopup(is_user_logged);
 		var link_url		=	base_url+"question";
 		var questionId 		=	$(this).attr('btn-question-id');
 		var answerText 		=	$("#answer_text_plain").val();
 		var post_data		=	[
	 		{name: 'AJAX',value: 'AJAX'},
	 		{name: 'TYPE',value: 'ACTION'},
	 		{name: 'ACTION',value: 'ADD_ANSWER'},
		...
		<div class="ed_courses">
			<div>
					...
					if( !empty($question_details)) {
							...
							$tags 					=	isset($question_details['tags'])?$question_details['tags']:'';
							...
							$qMimeType 					=	isset($question_details['mimeType'])?$question_details['mimeType']:"text/plain";
					?>
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="width: 99%;">
						...
						<div class="col-md-2" style="margin: 0px;float: right;"><a href="<?=base_url().'question/create/text/'.$course['id'] ?>"><button class="primary" title="Create New Question" style="background: #0364b3;color: #fff;height: 40px;border-radius: 10px;">New Question <i class="fa fa-plus"></i></button></a></div>
						</div>
						<div class="row q-list-item">
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 q-list-container" style="padding: 2px;">
									...
									<?php 
										if($questionType == 'text') {
											...
											if($mcq&&is_array($options)) {
												...
												echo '<li class="option-save-btn"><i class="fa fa-save ans-submit mcq-ans-submit" question-mime-type="'.$qMimeType.'" btn-question-id="'.$questionId.'" title="Click to save"></i></li>';
												echo "</ul>";
											}
								...
								<div class="row q-list-right q-list-tags-single" >
									...
									<?php if(isset($user_details['uid']) && $user_details['uid'] == $question_details['userId'] && ($questionType == "text" ||  $questionType == "image")) { ?>
										...
										<div class="row">
											<div class="col-md-6">
												...
												<ul class="nav nav-tabs" role="tablist"> 
													<li role="textanswer" class=""><a href="#textanswer" data-toggle="tab" aria-controls="textanswer" role="tab" >Text</a></li>
													<li role="htmlquestion" class="active"><a href="#htmlquestion" aria-controls="htmlquestion" data-toggle="tab" role="tab">HTML</a></li>
													<li role="latexquestion" class=""><a href="#latexquestion" data-toggle="tab" aria-controls="latexquestion" role="tab" >Latex</a></li>
												</ul>
												<!-- Tab panes -->
												<div class="tab-content">
													<div role="tabpanel" class="tab-pane active" id="htmlquestion">
							...
							<br>
													...
													<div role="tabpanel" class="tab-pane" id="textanswer">
														<div class="ed_dashboard_inner_tab">
															<div class="row" style="margin-left: 0px !important;">
																<div class="add-form">
							
							<label>Answer:</label>
							<textarea class="form-control question-text-area" id="answer_text_plain" name="answer_text_plain"></textarea>
							<button class="btn btn-primary single-ans-submit-plain" btn-question-id="<?=$questionId?>" style="float: right;margin: 10px;">Add answer</button>
							<br>
						</div>
															</div>
														</div>
													</div>
													<div role="tabpanel" class="tab-pane" id="latexquestion">
														<div class="ed_dashboard_inner_tab">
															<div class="row" style="margin-left: 0px !important;">
																<div class="add-form">
							<label>Answer:</label>
							<textarea class="form-control question-text-area" id="answer_text_latex" name="answer_text_latex"></textarea>
							<button class="btn btn-primary single-ans-submit-latex" btn-question-id="<?=$questionId?>" style="float: right;margin: 10px;">Add answer</button>
							<br>
						</div>
															</div>
														</div>
											...
											<div class="col-md-6">
												<div class="user-answer-dup">
													<div class="row" style="margin-top: 75px;" id="user_answer_console">
...
<script type="text/javascript">
	...
	// });
	$(document).on("change","#answer_text_latex",function() {
		var latex_str = $(this).val();
		loadLatex(latex_str);
	});
	$(document).on("keyup","#answer_text_latex",function() {
		var latex_str = $(this).val();
		loadLatex(latex_str);
	});
